...
# 📧 Admin Email Setup 
ADMIN_EMAIL = "facedetection2345@gmail.com"
...
def register():
    try:
        ...
        # ✅ Validate Face Presence by Downloading Image from Firebase URL
        try:
            import requests
            print("📥 Downloading face image from Firebase URL...")

            response = requests.get(face_url)
            if response.status_code != 200:
                print(f"⚠️ Failed to download image. Status Code: {response.status_code}")
                return jsonify({"success": False, "message": "Failed to download face image from Firebase."}), 400
            ...
            if captured_img is None:
                print("⚠️ captured_img is None after downloading from Firebase.")
                return jsonify({"success": False, "message": "Invalid face image provided."}), 400
            ...
            # ✅ Friendly Error Responses Based on Specific Error Patterns
            if "Face could not be detected in numpy array" in error_message:
                user_friendly_message = "❗ Face not detected. Please ensure your face is clearly visible and try again."
            elif "Multiple faces detected" in error_message:
                user_friendly_message = "❗ Multiple faces detected. Please ensure only your face is visible in the frame."
            elif "enforce_detection" in error_message:
                user_friendly_message = "❗ Face detection enforcement failed. Try adjusting your position or lighting."
            else:
                user_friendly_message = "❗ An unexpected error occurred during face detection. Please try again."

            return jsonify({"success": False, "message": user_friendly_message}), 400

        # ✅ Insert user to DB after successful validation
        ...
        return jsonify({
            "success": True,
            "message": "User registered successfully and PDF card created!",
